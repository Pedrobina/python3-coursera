'''
It may help for this assignment to be slightly familiar with with the card game Pokemon, but it's not necessary.
The key element of the game that we are simulating here is that a creature can be "trained", which enhances some its capabilities (health, defense, attack, etc.)
If trained enough, a creature can "evolve" into a different creature.
The class `Pokemon` is provided below. Each instance describes a Pokemon and its leveling and evolving characteristics.
...
Then create an instance of this class with the name as "Belle". Assign this instance to the variable p1.
The hidden test that runs when you submit your assignment will invoke your .action() method and check that it returns the correct string.
'''
